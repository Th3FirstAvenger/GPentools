...
class AtomicService(BaseService):
    ...
    def __init__(self):
        self.log = self.add_service('cheat_sheet_svc', self)
        ...
        self.atomic_dir = os.path.join('plugins', 'cheat_sheet')
    ...
    async def populate_data_directory(self, path_yaml=None):
        ...
        if not path_yaml:
            path_yaml = os.path.join(self.repo_dir, 'cheat_sheets', '**', 'T*.yaml')
